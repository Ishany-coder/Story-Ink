...
// Reference photo + caption uploaded in the wizard's outline step.
// Each entry is anchored by a client-generated id so the Stage-1
// script (and downstream image generation) can reference it
// unambiguously. The photoUrl lands in Supabase Storage via
// /api/upload and must clear isAllowedContentUrl on the server.
...
// V2 script (output of Stage 1) is now Zod-validated. The schema and
// inferred types live in `@/lib/script-schema`; import + re-export so
// the rest of the codebase can keep importing `Script` / `ScriptPage`
// from `@/lib/types` AND so the local `Story` interface below can
// reference them.
import type {
  Script as _Script,
  ScriptPage as _ScriptPage,
  MemoryUsage as _MemoryUsage,
} from "./script-schema";
export type Script = _Script;
export type ScriptPage = _ScriptPage;
export type MemoryUsage = _MemoryUsage;
